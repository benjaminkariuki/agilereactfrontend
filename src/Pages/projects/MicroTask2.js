...
const MicroTask = ({
  ...
  .filter((user) => {
    // Check if the user's department (in lowercase) is equal to "implementation" (in lowercase)
    return user.user?.department.toLowerCase() === "implementation";
  })
  .map((user, index) => ({
    key: index,
    label:
  ...
  ?.filter((user) => user.status === "active" && user.user?.department?.toLowerCase() !== "implementation")
  .map((user, index) => ({
    key: index,
    label:
      user.user?.firstName +
